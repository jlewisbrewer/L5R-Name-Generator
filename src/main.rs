...
fn get_file_contents(path_name: &str)-> Vec<Name> {
...
fn generate_name<'a>(sex: &str)-> Name {
...
fn update_file(sex: &str, name_type: &str, info: Name) {
    match sex.to_lowercase().as_ref() {
        "male" => {
...
fn write_to_file(file_name: &str, input: Name) {
...
fn user_generate_name() {
    ...
    match io::stdin().read_line(&mut input) {
        Ok(n) => { number = input.trim().parse::<usize>().unwrap();}
        Err(error) => println!("error: {}", error),
    };
...
fn choose_clan_name(clan: &str) -> Name {
    
    let mut input = String::new();
    let file_name = format!("data/clan_names/{}.txt", clan);
    let mut clan_names : Vec<Name> = get_file_contents(&file_name);
...
fn main() {
    ...
    // choose_clan_name("crane");
    loop {
        println!("Input [g] to generate name or [a] to add name.");
        let mut input = String::new();
